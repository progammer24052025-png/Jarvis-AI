...
import edge_tts
from app.models import ChatRequest, ChatResponse, TTSRequest, UploadResponse
...
# ================================================================
# NEW FEATURES: Export, Search, Statistics, File Upload
# ================================================================
...
async def search_chats(q: str):
    """Search within all chat sessions."""
    if not chat_service:
        raise HTTPException(status_code=503, detail="Chat service not initialized")
    if not q or len(q) < 2:
...
async def get_stats():
    ...
    try:
        ...
        total_messages = sum(len(msgs) for msgs in chat_service.sessions.values())
        user_messages = sum(1 for msgs in chat_service.sessions.values() for msg in msgs if msg.role == "user")
        assistant_messages = sum(1 for msgs in chat_service.sessions.values() for msg in msgs if msg.role == "assistant")
        
        # Calculate total characters
        total_chars = sum(len(msg.content) for msgs in chat_service.sessions.values() for msg in msgs)
...
async def upload_file(file: UploadFile):
    """Upload a file for learning."""
    try:
        contents = await file.read()
        text = contents.decode("utf-8")
        
        # Save to learning_data directory
        ...
        return UploadResponse(status="success", message=f"File {filename} uploaded and indexed")
    except Exception as e:
